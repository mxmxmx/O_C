...
- **miniature** (!) switch (pcb mount); on/off (SPDT) : 1x **OR** if to switch bipolar/unipolar output by jumper: 1x3 pin header (2.54mm) + jumper) **OR** if you want a different range/the option to adjust the offset properly: 50k trimpot (cermet type)
